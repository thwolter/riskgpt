...
def available() -> List[str]:
    ...
    ``src.chains`` package to ensure that all decorators run and
    ...
    if not _CHAIN_REGISTRY:
        try:  # pragma: no cover - reload is best effort
            ...
            importlib.reload(src.chains)
            modules = [
                "src.chains.get_categories",
                "src.chains.get_risks",
                "src.chains.check_definition",
                "src.chains.get_drivers",
                "src.chains.get_assessment",
                "src.chains.get_mitigations",
                "src.chains.prioritize_risks",
                "src.chains.cost_benefit",
                "src.chains.get_monitoring",
                "src.chains.get_opportunities",
                "src.chains.communicate_risks",
                "src.chains.bias_check",
                "src.chains.get_correlation_tags",
            ]
