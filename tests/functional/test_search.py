...
async def test_google_search():
    ...
    request = SearchRequest(query="artificial intelligence", scope=ScopeEnum.LINKEDIN)
...
async def test_wikipedia_search():
    ...
    request = SearchRequest(
        query="artificial intelligence", scope=ScopeEnum.REGULATORY, max_results=5
    )
...
async def test_combined_search(monkeypatch):
    ...
    request = SearchRequest(
        query="artificial intelligence", scope=ScopeEnum.LINKEDIN, max_results=5
    )
...
def search_request() -> SearchRequest:
    ...
    return SearchRequest(query="test query", scope=ScopeEnum.NEWS, max_results=5)
...
async def test_compare_context_aware_modes():
    ...
    with (
        ...
        request = SearchRequest(query=query, scope=ScopeEnum.NEWS, max_results=5)
